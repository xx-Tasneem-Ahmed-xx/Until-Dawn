...
our::Mesh* our::mesh_utils::loadOBJ(const std::string& filename) {
    ...
    std::string material_base_dir = std::filesystem::path(filename).parent_path().string();
    if(material_base_dir.empty()) material_base_dir = ".";
    ...
    for (const auto &shape : shapes) {
        for (const auto &index : shape.mesh.indices) {
            ...
            if((3 * index.vertex_index + 2) < static_cast<int>(attrib.colors.size())){
                vertex.color = {
                        static_cast<unsigned char>(attrib.colors[3 * index.vertex_index + 0] * 255),
                        static_cast<unsigned char>(attrib.colors[3 * index.vertex_index + 1] * 255),
                        static_cast<unsigned char>(attrib.colors[3 * index.vertex_index + 2] * 255),
                        255
                };
            } else {
