...
class Menustate : public our::State
{
    ...
    void updateButtonLayout(const glm::ivec2 &frameBufferSize)
    {
        float scaleX = frameBufferSize.x / ReferenceWidth;
        float scaleY = frameBufferSize.y / ReferenceHeight;

        buttons[0].position = {395.0f * scaleX, 545.0f * scaleY};
        buttons[0].size = {425.0f * scaleX, 40.0f * scaleY};

        buttons[1].position = {409.0f * scaleX, 610.0f * scaleY};
        buttons[1].size = {400.0f * scaleX, 40.0f * scaleY};
    }
    ...
    void onInitialize() override
    {
        ...
        updateButtonLayout(getApp()->getFrameBufferSize());
    }
